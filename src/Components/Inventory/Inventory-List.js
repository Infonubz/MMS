...
export default function InventoryList({currentItems}) {
  ...
  const columns = [
      {
        ...
        dataIndex: "image",
        align: "center",
        render: (image) => <span className="flex h-[3vw] w-[7vw] justify-center pl-[2vw]">{image}</span>,
      },
      {
      ...
      },
      {
        ...
        render: (poNumber) => (
          <span className="text-[0.9vw]">{poNumber}</span>
        ),
      },
      {
      ...
      {
        title: <span className="text-[1vw]">Available Stock</span>,
        dataIndex: "available",
        align: "center",
        sorter: (a, b) => a.available?.localeCompare(b.available),
        render: (available) => <span className="text-[0.9vw]">{available}</span>,
      },
      {
        title: <span className="text-[1vw]">Received Stock</span>,
        dataIndex: "received",
        align: "center",
        sorter: (a, b) => a.received?.localeCompare(b.received),
        render: (received) => <span className="text-[0.9vw]">{received}</span>, 
      },
      {
        title: <span className="text-[1vw]">Last Updated</span>,
        dataIndex: "lastupdated",
        align: "center",
        sorter: (a, b) => a.lastupdated?.localeCompare(b.lastupdated),
        render: (lastupdated) => <span className="text-[0.9vw]">{lastupdated}</span>, 
      },
      {
        title: <span className="text-[1vw]">Stock Status</span>,
    ...
    return (
        <>
          <div className="mt-[0.4vw] h-full w-full">
            <ConfigProvider
              theme={{
                components: {
                  Table: {
