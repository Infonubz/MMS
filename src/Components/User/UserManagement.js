...
export default function UserManagement() {
  ...
  const [tabName, setTabName] = useState("Vendor");
  ...
  const [state, setState] = useState([
    {
      startDate: new Date(), // Set current date as default
      endDate: new Date(), // Set current date as default
      key: "selection",
    },
  ]);
  ...
  const dataSource = [
    {
      key: 1,
      id: 25561,
      profile: "Edward King Isabella George",
      email: "edward@gmail.com",
      mobile: "923-454-6576",
      contactPerson: "Edward King Isabella George",
      createdDate: "3rd Dec 2024",
      status: "Active",
    },
    {
      key: 2,
  ...
  const employeeData = [
    {
      key: 1,
      id: 25561,
      details: "Edward King Isabella George",
      email: "edward@gmail.com",
      mobile: "923-454-6576",
  ...
  const userData = tabName === "Vendor" ? dataSource : employeeData;
  const pageCount = Math.ceil(userData?.length / itemsPerPage);

  // Slice dataSource for pagination
  const currentItems = userData?.slice(itemOffset, itemOffset + itemsPerPage);
  const goToFirstPage = () => {
    setItemOffset(0);
    setPageNumber(1);
  };
  ...
  const handleDone = () => {
    setIsModalOpen(false);
    setIsDateSelected(true); // Mark date as selected after Done
  };
    ...
    <div className="">
      <div className="flex flex-row gap-[2vw] h-[4vh] justify-between">
        <div className="flex flex-row">
          <h1 className="text-[1.5vw] font-semibold">USER MANAGEMENT</h1>
          <span className="text-[1.3vw] font-semibold pl-[1vw]">-</span>
        </div>
        <div
          className={` cursor-pointer ml-[1vw] ${
            tabName === "Vendor"
              ? "border-b-[0.2vw] font-bold border-[#000000]"
              : ""
          } `}
        >
          <h1 className="text-[1.2vw]" onClick={() => setTabName("Vendor")}>
            Vendor
          </h1>
        </div>
        <div
        ...
        >
          <h1 className="text-[1.2vw]" onClick={() => setTabName("Employee")}>
            Employee
          </h1>
        </div>
        <div className="flex gap-[2vw] justify-end ml-auto">
          ...
          <div>
            <div>
              <input
                ...
                style={{ display: "none" }}
              />
              <button className="bg-[#ffffff] border border-[#ebebeb] shadow-md shadow-[#d8d6d6] flex px-[0.75vw] h-[1.8vw] justify-center gap-[0.5vw] items-center rounded-[0.6vw]">
          ...
          <div>
            <button
              className="bg-[#3348FF] border border-[#3348FF] shadow-md shadow-[#c4c0c0] flex px-[0.75vw] h-[1.8vw] justify-center 
                            gap-[0.5vw] items-center rounded-[0.7vw]"
            >
              <IoAddOutline size={"1.3vw"} color="#FFFFFF" />
              <span className="font-bold text-white text-[0.7vw]">
                {tabName === "Vendor" ? "Add Vendor" : "Add Employee"}
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-row bg-[#F9F9F9] mt-[1vh] h-[5.5vh] rounded-[1vw] justify-between p-[0.8vh]">
        <div className="flex">
          <div className="relative flex items-center w-[13.85vw]">
            ...
            <input
              type="text"
              className="bg-white outline-none text-[0.75vw] pl-[3vw] pr-[3vw] w-[15vw] h-[4vh] border-[0.1vw] border-[#dddddd] rounded-[0.7vw] shadow-md"
        ...
        <div className="flex gap-[1.5vw] justify-end ml-auto">
          <div className="">
            ...
            >
              <div
                ...
                className="flex flex-row h-[4vh] w-[15vw] rounded-[0.65vw] p-[0.3vw] items-center shadow-md placeholder-black justify-between cursor-pointer border-[0.1vw] bg-[#ffffff] border-[#e5e7eb]"
              >
                <div>
          ...
          <div className="">
            ...
            >
              <Select
                className="h-[4vh] relative flex justify-center items-center w-[10vw] rounded-[1vw] shadow-md placeholder:text-[1vw]"
                allowClear
                options={[
                  {
                    value: 1,
      ...
      <div className="flex-1 h-[68vh]">
        {tabName === "Vendor" ? (
          <div>
            <Vendor setIsModalPopup={setIsModalPopup} setRowCount={setRowCount} currentItems={currentItems} />
          </div>
        ) : (
          <div>
            <Employee setIsModalPopup={setIsModalPopup} setRowCount={setRowCount} currentItems={currentItems} />
          </div>
        )}
      </div>
      <div className="flex mt-[9.7vh] bg-[#f9f9f900] justify-between items-center">
        <div className="flex ml-[2vw]">
          <p className="text-[0.8vw]">
            Showing {itemOffset + 1} -{" "}
            {Math.min(itemOffset + itemsPerPage, userData?.length)} of{" "}
            {userData?.length} items
          </p>
        </div>
        ...
        <div className="flex flex-row gap-[0.5vw] bg-[#f9f9f900] items-center pr-[0.5vw]">
          ...
          <button
            className="bg-[#3348FF] text-[0.8vw] text-white border border-[#3348FF] shadow-md shadow-[#d8d6d6] flex px-[0.3vw] h-[1.5vw] justify-center items-center rounded-[1vw]"
        ...
        <div className="flex w-full">
          <div className="flex flex-col w-full">
            ...
            <div className="footer-buttons flex justify-end gap-[1vw] mt-2">
              ...
              <button
                className="bg-[#3348FF] text-[0.8vw] text-white font-bold border border-[#3348FF] shadow-md shadow-[#d8d6d6] flex px-[0.75vw] h-[1.3vw] justify-center items-center rounded-[0.6vw]"
                onClick={handleDone}
              >
                Done
              </button>
