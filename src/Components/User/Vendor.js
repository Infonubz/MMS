...
export default function Vendor({ setIsModalPopup, setRowCount, currentItems }) {
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div className="flex flex-row p-[0.6vh] pl-[1vw] gap-2">
          <span className="flex h-[2vw] w-[2vw]">
            <img alt="profile" src={profile} />
          </span>
          <span className="flex flex-col">
            {record.profile?.length > 20 ? (
              <Tooltip
                placement="top"
              ...
              >
                <p className="text-[0.85vw] font-bold">
                  {record.profile?.slice(0, 20)}...
                </p>
              </Tooltip>
            ) : (
              <p className="text-[0.85vw] font-bold">{record.profile}</p>
            )}
    ...
    {
      title: <span className="text-[0.95vw] font-normal">Vendor ID</span>,
      dataIndex: "id",
      align: "center",
        ...
        contactPerson?.length > 20 ? (
          ...
          >
            <div className="text-[0.85vw] pl-[2.5vw]">
              {contactPerson?.slice(0, 20)}...
            </div>
          </Tooltip>
        ) : (
          <div className="text-[0.85vw] pl-[2.5vw]">{contactPerson}</div>
        ),
      //<span className="text-[0.85vw]">{contactPerson}</span>,
    },
    {
